@extends('layout.app')
@section('content')
    <h1 class="h2">Atualizar contato</h1>
    @if (session('success'))
    ...
    <form action="{{route('admin.contact.update',[$contact->id])}}" method="post" class="m-3">
        @csrf
        @method('PUT')
        <div class="row">
        ...
        <div class="row">
            <div class="form-group col-md-10">
                ...
                <input type="text" class="form-control" id="Telefone" name="telefone" value="{{$contact->telefone}}">
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-10">
                <label for="endereco">Endereço</label>
                <input type="text" class="form-control" id="endereco" name="endereco" value="{{$contact->endereco}}">
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-10">
                <label for="endcepereco">CEP</label>
                <input type="text" class="form-control" id="cep" name="cep" onchange="buscarenEndereco(this.value)" value="{{$contact->cep}}">
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-10">
        ...
        <div class="row">
            <div class="form-group col-md-10">
                <input type="hidden" id="lat" name="lat" value="{{$contact->lat}}">
                <input type="hidden" id="long" name="long" value="{{$contact->log}}">
